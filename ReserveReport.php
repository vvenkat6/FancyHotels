...
$query = "select MONTH(RESERVATION.Start_date),RESERVATION_HAS_ROOM.Location, count(RESERVATION_HAS_ROOM.Location) from RESERVATION_HAS_ROOM inner join RESERVATION on RESERVATION.ReservationID = RESERVATION_HAS_ROOM.ReservationID where MONTH(RESERVATION.Start_date) in (8,9) and not Is_cancelled group by MONTH(RESERVATION.Start_date),RESERVATION_HAS_ROOM.Location";
$result = mysql_query($query);

if (mysql_num_rows($result) > 0) {
        echo "<table><tr><th>Month</th><th>Location</th><th>Number of Reservations</th></tr>";
		...
		if($row['MONTH(RESERVATION.Start_date)'] == '8'){
			...
			echo "<tr><td>September</td>";
                ...
                echo "<td>".$row["Location"]."</td><td>".$row["count(RESERVATION_HAS_ROOM.Location)"]."</td></tr>";
        }
echo "</table>";
}
?>
